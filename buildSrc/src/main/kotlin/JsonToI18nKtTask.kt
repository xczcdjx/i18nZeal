...
""".trimIndent()
    ...
    private fun buildKtFile(
        ...
        return buildString {
            ...
            appendLine()
            appendLangEnum(locales = translations.keys.toList())
            appendLine()
            appendLine("/**")
            ...
            translations.keys.forEach { locale ->
                val langName = locale.toLangName()
                val providerName = providerObjectName(objectName, locale)

                appendLine("        Lang.$langName to $providerName.map,")
            }
            ...
            appendLine("        fallback: String = key.orEmpty()")
            appendLine("    ): String {")
            appendLine("        if (key.isNullOrBlank()) return \"\"")
            appendLine()
            appendLine("        val text = langMap[lang]?.get(key) ?: fallback")
            appendLine()
    ...
    private fun StringBuilder.appendLangEnum(
        locales: List<String>
    ) {
        appendLine("enum class Lang(val code: String) {")

        locales.forEachIndexed { index, locale ->
            val enumName = locale.toLangName()
            val suffix = if (index == locales.lastIndex) "" else ","

            appendLine("    $enumName(\"${escapeKotlinString(locale)}\")$suffix")
        }
