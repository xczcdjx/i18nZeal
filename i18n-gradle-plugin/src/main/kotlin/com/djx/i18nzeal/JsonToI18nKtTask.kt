...
""".trimIndent()
    ...
    private fun buildKtFile(
        ...
        return buildString {
            ...
            appendLine(" * Copyright (c) 2026 xczcdjx.")
            appendLine(" */")
            appendLine()
            appendLine("package $packageName")
            appendLine()
            appendLine("import com.djx.i18n.runtime.export.Locale")
            appendLine("import com.djx.i18n.runtime.formatI18nText")
            ...
            translations.keys.forEach { locale ->
                ...
                appendLine("        $langName.code to $providerName.data,")
            }
            ...
            appendLine()
            appendLine("        val index = keyIndex(key)")
            appendLine("        val text = index?.let { langMap[locale.code]?.getOrNull(it) } ?: fallback")
            appendLine()
            appendLine("        if (args.isEmpty()) return text")
            appendLine("        if (!text.contains(\"{\")) return text")
            appendLine()
            appendLine("        return formatI18nText(text, *args)")
            appendLine("    }")
            appendLine()
            appendLine("    private fun keyIndex(key: String): Int? = when (key) {")

            keys.forEachIndexed { index, key ->
                appendLine("        I18nKeys.${key.toFieldName()} -> $index")
            }
    ...
    private fun StringBuilder.appendLangObject(
        ...
        appendLine("    val data = arrayOf(")

        keys.forEach { key ->
            val value = data[key].orEmpty()

            appendLine("        \"${escapeKotlinString(value)}\",")
        }
